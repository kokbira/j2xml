<?php
/**
 * @version		15.6.251 libraries/eshiol/j2xml/importer.php
 * 
 * @package		J2XML
...
class J2XMLImporter
{
	static function import($xml, $params)
	{
		...
		*/
		...
		$dispatcher->trigger('onAfterImport', array($option.'.article', $ids));
/*
		foreach ($ids as $id)
		{
			$table = JTable::getInstance('content');
			$table->load($id);
			$dispatcher->trigger('onContentAfterSave', array($option.'.article', $table, false));
		}
		
		require_once JPATH_SITE . '/components/com_content/helpers/route.php';
		$newUrl = ContentHelperRoute::getArticleRoute($table->id.':'.$table->alias, $table->catid, $table->language);
		if (isset($table->sourcelist))
			foreach ($table->sourcelist as $v)
				$redirect[html_entity_decode(trim($v))] = $newUrl;
			elseif (isset($table->source))
				$redirect[$table->source] = $newUrl;
		// rebuild internal link
		if ($rebuild_links)
		{
			foreach ($ids as $id)
			{
				$table = JTable::getInstance('content');
				$table->load($id);
				foreach($redirect as $oldUrl => $newUrl)
				{
					$table->introtext = str_replace ($oldUrl, $newUrl, $table->introtext);
					$table->fulltext = str_replace ($oldUrl, $newUrl, $table->fulltext);
				}
				$table->store();
			}
		}
